...
testing_data = np.ndarray(shape=(30,5), dtype = "float")
...
for year in testing_years:
	...
	with open(year+"Stats.csv","r") as statfile:
		for idx,line in enumerate(statfile):
			...
			testing_data[idx+(fcounter*30)][0] = np.float(items[7])
			testing_data[idx+(fcounter*30)][1] = np.float(items[10])
			testing_data[idx+(fcounter*30)][2] = np.float(items[13])
			testing_data[idx+(fcounter*30)][3] = np.float(items[19])
			testing_data[idx+(fcounter*30)][4] = np.float(items[22])
	fcounter+=1
