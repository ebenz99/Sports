...
training_data = np.ndarray(shape=(90,7,1), dtype = "float")
...
for year in training_years:
	...
	with open(year+"Stats.csv","r") as statfile:
		for idx,line in enumerate(statfile):
			...
			training_data[idx+(fcounter*30)][4][0] = np.float(items[21])
			training_data[idx+(fcounter*30)][5][0] = np.float(items[16])
			training_data[idx+(fcounter*30)][6][0] = np.float(items[23])
	fcounter+=1
...
testing_data = np.ndarray(shape=(30,7,1), dtype = "float")
...
for year in testing_years:
	...
	with open(year+"Stats.csv","r") as statfile:
		for idx,line in enumerate(statfile):
			...
			testing_data[idx+(fcounter*30)][4][0] = np.float(items[21])
			testing_data[idx+(fcounter*30)][5][0] = np.float(items[16])
			testing_data[idx+(fcounter*30)][6][0] = np.float(items[23])
	fcounter+=1
